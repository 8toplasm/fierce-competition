...
execute as @a[scores={cutscene_logic=1}] run rotate @s facing entity @n[type=cobblemon:npc, name=Herh]
execute as @a[scores={cutscene_logic=1}] run gamemode survival
runmolang "q.npc.start_battle(q.player);" @p @n[type=cobblemon:npc, name=Herh]
